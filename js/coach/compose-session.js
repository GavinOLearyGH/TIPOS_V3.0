...
function choosePractice({context,state,avoid=[],preferredIds=[]}){const mapped=SESSION_CONTEXTS[context]?.tip9;let pool=TIP9_PRACTICES.filter(p=>!avoid.includes(p.id)&&(!mapped||p.contexts.includes(mapped)));if(!pool.length)pool=TIP9_PRACTICES.filter(p=>!mapped||p.contexts.includes(mapped));return pool.map(p=>({practice:p,score:rankPractice(p,state,preferredIds)})).sort((a,b)=>a.score-b.score)[0]?.practice||null;}
...
export function composeTodaySession({minutes=30,context='anywhere'}={}){
  const state=TIPState.get(); const duration=normalizeMinutes(minutes); const safeContext=SESSION_CONTEXTS[context]?context:'anywhere';
  const suggestion=getTIPSuggestion(); const tip7=getTIP7Status(); const blocks=[]; const used=[]; const preferred=suggestionPracticeIds(suggestion);
  let adaptedFromPriority=false;

  if(duration===7){
    if(suggestion?.kind==='tip7'&&tip7.canStart){blocks.push({kind:'tip7',title:tip7.nextDay.theme,day:tip7.nextDay.day,estimate:7});}
    else {
  ...
  const baseReason=suggestion?.reason||'Built from your current progress and recent Journal.';
  const reason=adaptedFromPriority&&safeContext!=='anywhere'
    ? `${baseReason} That exact work is not available in ${contextLabel}, so TIP adapted the session to useful work you can do here.`
    : baseReason;
  return {
    id:`session_${Date.now()}`, title:"Today's Session", minutes:duration, context:safeContext, contextLabel,
    reason, focus:adaptedFromPriority?null:(suggestion?.title||null), adaptedFromPriority,
