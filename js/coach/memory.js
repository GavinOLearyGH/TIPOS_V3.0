...
function trendFromHistory(history=[]){
  ...
  const weighted=recent.reduce((sum,item)=>sum + item.signal*item.weight,0);
  if(weighted>=4) return 'Improving';
  if(weighted<=-4) return 'Sliding';
  return 'Stable';
}
...
export function buildMemoryFromJournal(journal=[]){
  ...
  for(const signal of signals){
    ...
    const impact=signal.signal>=0?weight:-weight;
    topic.evidence+=weight;
    topic.score=clamp(topic.score+impact,-100,100);
    if(signal.signal>=0) topic.positive+=weight; else topic.negative+=weight;
    topic.lastObserved=signal.at || null;
    topic.history.push({at:signal.at,signal:signal.signal,weight,source:signal.source,note:signal.note,ref:signal.ref});
  ...
  return {
    ...
    summary:{
      known:known.length,
      totalEvidence:known.reduce((sum,t)=>sum+t.evidence,0),
