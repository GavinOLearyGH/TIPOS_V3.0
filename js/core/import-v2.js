...
function uniqueKey(entry) { return `${entry.source}|${entry.type}|${entry.createdAt}|${entry.title}`; }
function importRounds(rounds) {
  return (Array.isArray(rounds) ? rounds : []).map(round => createJournalEntry({
    ...
    createdAt: iso(round.createdAt || round.at || round.date), type:'round', source:'v2-import',
    title: round.course || round.courseName || round.title || 'Round', dimensions:['skill'],
...
function importSessions(sessions) {
  return (Array.isArray(sessions) ? sessions : []).map(session => createJournalEntry({
    ...
    createdAt: iso(session.createdAt || session.at || session.date || session.completedAt), type:'practice', source:'v2-import',
    title: session.focus || session.title || session.type || 'Practice', dimensions:[],
    metrics:{ duration:session.duration || session.minutes }, reflection:{ text: session.note || session.detail || session.feeling || session.reflection || '' }
  }));
}
function importNotebook(notes) {
  return (Array.isArray(notes) ? notes : [])
    ...
    .map(note => createJournalEntry({
      ...
      source:'v2-import', title: note.title || note.type || 'Golf Note',
...
export function detectV2Data() {
  const rounds=read(KEYS.rounds,[]), sessions=read(KEYS.sessions,[]), notes=read(KEYS.notebook,[]);
  const counts={rounds:Array.isArray(rounds)?rounds.length:0,sessions:Array.isArray(sessions)?sessions.length:0,notes:Array.isArray(notes)?notes.length:0};
...
export function importV2FromThisDevice() {
  ...
  const candidates=[...importRounds(read(KEYS.rounds,[])),...importSessions(read(KEYS.sessions,[])),...importNotebook(read(KEYS.notebook,[]))];
  const player=read(KEYS.player,{}), legacyMemory=read(KEYS.memory,null);
  let added=0;
  TIPState.update(state => {
    ...
    for(const entry of candidates){
      ...
      state.journal.push(entry); existing.add(key); added++;
    }
    ...
    state.meta.v2ImportCount=Number(state.meta.v2ImportCount||0)+added;
    if(legacyMemory) state.meta.hasLegacyMemoryForFutureMigration=true;
    return state;
  },'state:import-v2');
  return { imported:added, examined:candidates.length, counts:found.counts };
}
