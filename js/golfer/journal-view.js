...
function metricLine(entry) {
  ...
  if (entry.type === 'tip9') {
    ...
    const context=String(entry.context?.practiceContext||'').replace('range','Range').replace('bay','Hitting Bay').replace('green','Putting Green').replace('short','Short Game').replace('noball','No Ball');
...
function entryHTML(entry) {
  ...
  return `
    <article class="card journal-card">
      <div class="card-top">
        <div><div class="eyebrow">${esc(entry.type)} ${imported}</div><h3>${esc(entry.title)}</h3></div>
...
export function renderGolfer() {
  ...
  return `
    ...
    <section class="section">
      ...
      ${entries.length ? `<div class="journal-preview">${entries.map(entryHTML).join('')}</div>` : `<div class="empty-state"><strong>Start with what happened.</strong><p>Add a round, practice, lesson, equipment change or simple note. TIP7 and TIP9 also write here automatically when you complete them.</p></div>`}
    </section>`;
}
