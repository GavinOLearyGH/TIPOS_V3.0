...
function metricLine(entry) {
  ...
  if (entry.type === 'tip9') {
    ...
    const parts=[result.practiceType,`Level ${result.level||1}`,context,score].filter(Boolean);
    return `<div class="journal-metrics">${parts.map(esc).join('<span>·</span>')}</div>`;
  }
  if (entry.type === 'session') {
    ...
    const parts=[duration,context,`${blocks} ${blocks===1?'activity':'activities'}`].filter(Boolean);
    return `<div class="journal-metrics">${parts.map(esc).join('<span>·</span>')}</div>`;
  }
...
function entryHTML(entry) {
  ...
  return `
    <article class="card journal-card">
      <div class="card-top"><div><div class="eyebrow">${esc(entry.type)} ${imported}</div><h3>${esc(entry.title)}</h3></div><small>${esc(prettyDate(entry.createdAt))}</small></div>
      ${metricLine(entry)}
      ...
      ${automatic ? `<div class="card-meta">Saved automatically by ${entry.source==='tip-session'?'TIP':esc(entry.source.toUpperCase())}.</div>` : `<div class="journal-actions"><button type="button" data-action="edit-entry" data-entry-id="${esc(entry.id)}">Edit</button><button type="button" data-action="delete-entry" data-entry-id="${esc(entry.id)}">Delete</button></div>`}
    </article>`;
}
export function renderGolfer() {
  ...
  return `
    ...
    <section class="section">
      ...
      ${entries.length ? `<div class="journal-preview">${entries.map(entryHTML).join('')}</div>` : `<div class="empty-state"><strong>Start with what happened.</strong><p>Add a round, practice, lesson, equipment change or simple note. TIP7, TIP9 and built sessions also write here automatically.</p></div>`}
    </section>`;
}
