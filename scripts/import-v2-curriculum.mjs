...
const sourceLessonCount=Number(curriculum.lessonCount||lessons.length);
const excludedCount=lessons.length-compact.length;
const report={
  ...
  excludedCount,
  coverage:{
    readyVisibleImported:compact.length,
    readyVisibleAvailable:eligible.length,
    readyVisiblePercent:eligible.length?Number((compact.length/eligible.length*100).toFixed(1)):0,
    allSourceLessonsPercent:sourceLessonCount?Number((compact.length/sourceLessonCount*100).toFixed(1)):0
  },
...
await fs.writeFile(path.join(OUT_DIR,'v2-practice-library.json'),JSON.stringify({version:'3.12-migration-3',source:SOURCE_URL,report,items:compact},null,2));
